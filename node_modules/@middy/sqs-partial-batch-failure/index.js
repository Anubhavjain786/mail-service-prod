...
const sqsPartialBatchFailureMiddleware = (opts = {}) => {
  ...
  const getQueueUrl = eventSourceARN => {
    const [,,,, accountId, queueName] = eventSourceARN.split(':'); // aws-sdk v2
  ...
  const deleteSqsMessages = async fulfilledRecords => {
    if (!(fulfilledRecords !== null && fulfilledRecords !== void 0 && fulfilledRecords.length)) return null;
    const Entries = getEntries(fulfilledRecords);
    const {
      eventSourceARN
    } = fulfilledRecords[0];
    const QueueUrl = getQueueUrl(eventSourceARN);
    const promises = []; // deleteMessageBatch only supports 10 messages at a time
    ...
    for (i = 0, j = Entries.length; i < j; i += chunk) {
      ...
      promises.push(client.deleteMessageBatch({
  ...
  const sqsPartialBatchFailureMiddlewareAfter = async request => {
    ...
    const rejectedReasons = getRejectedReasons(response); // If all messages were processed successfully, continue and let the messages be deleted by Lambda's native functionality
...
const getRejectedReasons = response => {
  ...
  const rejectedReasons = rejected.map(r => {
    ...
    return (_r$reason = r.reason) === null || _r$reason === void 0 ? void 0 : _r$reason.message;
  });
